...
#test realy--------------------------------------------------------------------------------------------
    ...
    def control_RL(self, pos, option, status): # control realy
        try:
            self.instrument.write_register(registeraddress=(2000 + (pos-1)*8 +(option-1)), value=status,
                                                number_of_decimals=0, functioncode=16, signed=False)
        except :
            pass
           
    # get status of relay - phản hồi trạng thái hiện tại của relay
    def get_status_RL(self, pos, option):
        try:
            data = self.instrument.read_registers(
                    registeraddress=(2000 + (pos-1)*8 +(option-1)), number_of_registers=1, functioncode=3)
            return data[0]	
        except :
            pass
